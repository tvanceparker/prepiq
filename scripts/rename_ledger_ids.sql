-- SQL script to rename ledger ID columns for clarity
-- Run this to update existing tables to use specific ledger ID names

-- 1. Rename eod_run_ledger.id to eod_ledger_id
ALTER TABLE eod_run_ledger 
CHANGE COLUMN id eod_ledger_id BIGINT UNSIGNED AUTO_INCREMENT;

-- 2. Drop existing forecast_run_ledger if it exists (will recreate with correct name)
DROP TABLE IF EXISTS forecast_run_ledger;

-- 3. Create forecast_run_ledger with forecast_ledger_id
CREATE TABLE forecast_run_ledger (
    ...
    -- Progress tracking
    menu_items_processed INT UNSIGNED NOT NULL DEFAULT 0,
