...
class InventoryService:
    ...
    async def get_inventory_discrepancy_history(
        ...
        *,
        ...
        if not is_full_service_tier(self.subscription_tier):
            raise Exception("Discrepancy history is only available for Full tier restaurants.")
    ...
    async def get_stock_movements(self, start_date: date, end_date: date, ingredient_id: int = None) -> list:
        ...
        """
        if not is_full_service_tier(self.subscription_tier):
            raise Exception("Stock Movements are only available for Full tier restaurants.")
