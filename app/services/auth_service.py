...
from app.utils.security import verify_password, create_access_token, create_device_token, decode_token_claims
...
class AuthService:
    ...
    async def authenticate_and_create_token(self, username: str, password: str):
        ...
        return user, token, subscription_tier, expires_in
    ...
    async def get_current_user_info(self, employee_id: int, restaurant_id: int, role_id: int):
        ...
        logger.info(f"[AuthService] Fetched user info + {len(permissions)} permissions for employee {employee_id}")
        
        return user, list(permissions)

    @log_method("User logout")
    async def logout(self, employee_id: int, restaurant_id: int, username: str):
        ...
        logger.info(f"[AuthService] User {username} (id={employee_id}) logged out")
