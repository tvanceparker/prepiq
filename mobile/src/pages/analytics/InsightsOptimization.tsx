...
const insights: Insight[] = [
  {
    id: 1,
    type: 'Waste',
    message: 'Tomatoes waste up 20% last week.',
    action: 'Review prep schedule.',
    severity: 'warning',
  ...
  },
  {
    id: 2,
    type: 'Profit',
    message: 'Chicken Breast margin 45%.',
    action: 'Promote dish.',
    severity: 'success',
  ...
  },
  {
    id: 3,
    type: 'Inventory',
    message: 'Basil below reorder point.',
    action: 'Create PO.',
    severity: 'critical',
  ...
  },
  {
    id: 4,
    type: 'Forecast',
    message: 'Dairy forecast accuracy dropped 15%.',
    action: 'Review model.',
    severity: 'warning',
...
export default function InsightsOptimization() {
  ...
  const visible = useMemo(
    () => insights.filter(i => (filter === 'All' || i.type === filter) && !dismissed.has(i.id)),
    [filter, dismissed]
  );
  return (
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      ...
      {visible.length === 0 && (
        <Text style={{ textAlign: 'center', color: '#666' }}>No insights.</Text>
      )}
      {visible.map(ins => (
        <View
          key={ins.id}
          style={{
            ...
            backgroundColor: severityColors[ins.severity],
            padding: 12,
            borderRadius: 8,
            marginBottom: 10,
          }}
        >
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <Text
              onPress={() => setDismissed(prev => new Set(prev).add(ins.id))}
              style={{ color: '#555' }}
            >
              ×
            </Text>
