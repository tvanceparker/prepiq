...
export default function SalesUploadWizard({ navigation }: any) {
  ...
  useEffect(() => {
    load().catch(e =>
      setSnackbar({ visible: true, message: `Failed to load: ${e?.message || e}` })
    );
  }, [load]);
  ...
  const submitAll = async (confirmOverwrite: boolean) => {
    ...
    try {
      ...
      await submit(sale_date, payloadEntries, !!confirmOverwrite);
      // Navigate back to overview and trigger a refresh + toast there
      const toast = confirmOverwrite ? 'Sales overwritten' : 'Sales uploaded';
      navigation.navigate('dashboard_daily-overview', { refresh: true, toast });
    } catch (e: any) {
      // Try to extract axios / server error details for visibility
      const status = e?.response?.status;
      const data = e?.response?.data || e?.message || String(e);
      const msg = status ? `${status}: ${JSON.stringify(data)}` : String(data);
      setSnackbar({ visible: true, message: msg });
      // eslint-disable-next-line no-console
      console.error('SalesUpload submit error', { e, status, data });
      setSubmitting(false);
    }
  };
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              <Button
                ...
                disabled={nonZeroEntries.length === 0 || submitting}
                loading={submitting}
              >
