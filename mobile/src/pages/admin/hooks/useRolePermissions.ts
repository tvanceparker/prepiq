...
export function useRolePermissions() {
  ...
  const mutationsLoading = syncMutation.status === 'pending' || deleteMutation.status === 'pending';
  return {
    roles: rolesQuery.data || [],
    permissions: permissionsQuery.data || [],
    loading: rolesQuery.isLoading || permissionsQuery.isLoading || mutationsLoading,
    error: rolesQuery.error || permissionsQuery.error || syncMutation.error || deleteMutation.error,
    syncData: (roles: any, deleted_roles: any) => syncMutation.mutateAsync({ roles, deleted_roles }),
