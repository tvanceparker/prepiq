...
export default function SystemAlertsBasic() {
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => {
        const color =
          item.level === 'Critical' ? '#dc2626' : item.level === 'Warning' ? '#d97706' : '#2563eb';
        return (
          <Card style={styles.card}>
            <Card.Title
              title={item.message}
              subtitle={`${item.level} • ${item.date}`}
            ...
            <Card.Content>
              <View style={styles.row}>
                ...
                <Chip
                  mode={item.resolved ? 'flat' : 'outlined'}
                  style={{ marginLeft: 8 }}
                  icon={item.resolved ? 'check-circle' : undefined}
                >
      ...
      ListEmptyComponent={
        <Text style={{ textAlign: 'center', marginTop: 40 }}>No system alerts.</Text>
      }
    />
  );
}
