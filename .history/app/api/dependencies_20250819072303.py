...
class CurrentUser:
    def __init__(self, username, restaurant_id, subscription_tier, employee_id, name, role_id):
        ...
        # Backwards-compat alias: some code expects `current_user.role`
        self.role = role_id
        logger.info(
            f"[Dependency] Constructing CurrentUser: {self.username}, {self.restaurant_id}, "
            f"{self.subscription_tier}, {self.employee_id}, {self.name}, {self.role_id}"
        )
