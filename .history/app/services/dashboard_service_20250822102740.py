...
class DashboardService:
    ...
    async def upload_sales_data(self, file: UploadFile, overwrite: bool = False):
        ...
        existing_sales = await self.sales_repo.sales_exist_for_dates(list(sale_dates))

        if existing_sales and not overwrite:
            raise HTTPException(
                status_code=409,
                detail="Sales already exist for one or more dates. Use overwrite=True to replace them."
            )

        if overwrite and existing_sales:
            await self.sales_repo.delete_sales_for_dates(list(sale_dates))
            await self.log_activity("overwrite_sales", list(sale_dates))
    ...
    async def upload_sales_entries(self, payload: EodSalesEntriesIn):
        ...
        # Overwrite semantics by date
        if payload.overwrite:
            await self.sales_repo.delete_sales_for_dates([sale_date])
            await self.log_activity("overwrite_sales_manual", {"sale_date": payload.sale_date})
        else:
            # if existing and not overwrite, raise 409 like file upload
            if await self.sales_repo.sales_exist_for_dates([sale_date]):
                raise HTTPException(status_code=409, detail="Sales already exist for that date. Use overwrite=true.")
