...
class EODService:
    ...
    async def finalize_end_of_day_summary(self, date: date, commit: bool = True) -> None:
        try:
            ...
            # Idempotency: if forecasts already exist covering this date for this restaurant, skip
            from app.repositories.forecasts_repo import ForecastRepository
            forecast_repo = ForecastRepository(self.db, self.restaurant_id)
            existing = await forecast_repo.get_forecasts_covering_date(date)
            if existing:
                logger.info(f"[EOD] Forecasts already exist for {date} for restaurant {self.restaurant_id}; skipping EOD to preserve idempotency.")
                return
