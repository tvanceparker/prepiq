...
export default function Sidebar({ tier, onNavigate }: Props) {
  ...
  useEffect(() => {
    if (Object.keys(open).length === 0) {
      ...
      sections.slice(0,2).forEach(s => { init[s.label] = true; });
      setOpen(init);
    }
  }, [sections]);
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        {sections.map(section => {
          const children = permissions.length === 0
              ...
              {isOpen && (
                <View style={styles.items}>
                  {children.map(child => {
                    ...
                    const isActive = nav.getState?.()?.routes?.some((r:any) => r.name === screenName);
                    return (
                      <TouchableOpacity key={child.path} style={[styles.itemBtn, isActive && styles.itemActive]} onPress={() => go(child.path)}>
                        <Text style={[styles.itemText, isActive && styles.itemTextActive]}>{child.name}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
        ...
        <View style={styles.footer}>
          <Button icon="logout" mode="outlined" onPress={logout} style={styles.logoutBtn}>Logout</Button>
        </View>
      </ScrollView>
    </View>
  );
