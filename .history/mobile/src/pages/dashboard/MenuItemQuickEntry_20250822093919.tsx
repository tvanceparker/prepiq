...
function Pill({ active, label, onPress }: { active: boolean; label: string; onPress: () => void }) {
  ...
  return (
    ...
    >
      <Text
        style={{ fontSize: 12, color: active ? (theme.colors.onPrimary as string) : (theme.colors.onSurface as string) }}
      >
        {label}
      </Text>
    </TouchableOpacity>
      ...
      {editing ? (
        <>
          ...
          <View style={{ flexDirection: 'row', marginTop: 4 }}>
            ...
            <TouchableOpacity onPress={() => setEditing(false)} style={smallBtn(theme.colors.surfaceVariant as string)}>
        ...
        <>
          ...
          <View style={{ flexDirection: 'row', marginTop: 6 }}>
            <TouchableOpacity onPress={() => setEditing(true)} style={smallBtn(theme.colors.primary as string)}>
              <Text style={{ color: theme.colors.onPrimary }}>Edit</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => onDelete(item.menu_item_id)} style={smallBtn(theme.colors.error as string)}>
