...
export default function SalesUploadWizard({ navigation }: any) {
  ...
  const { items: menuItems, channels: orgChannels, load, checkConflicts, submit } = useSalesUpload();

  useEffect(() => {
    load()
      .catch((e) => setSnackbar({ visible: true, message: `Failed to load: ${e?.message || e}` }));
  }, [load]);
