...
import { uploadSalesData, uploadSalesManual } from '../../../api/dashboard';
...
export default function BasicOverviewMobile({ data }: Props) {
  ...
  const handleManualSubmit = async () => {
    try {
      ...
      await uploadSalesManual({ sale_date: saleDate, overwrite, entries: [{ menu_item_id: menuItemId, quantity_sold: qty }] });
      setSheetOpen(false);
      setManualQty('');
      setManualMenuItemId('');
