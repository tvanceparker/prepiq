...
export default function BasicOverviewMobile({ data, navigation }: Props & { navigation?: any }) {
  ...
  const onUploadSales = () => {
    if (navigation && typeof navigation.navigate === 'function') {
      navigation.navigate('sales_upload-wizard');
      return;
    }
    setSheetOpen(true);
  };
  ...
  const handleManualSubmit = async () => {
    const entries = Object.entries(qtyById)
      ...
      .map(r => ({
        menu_item_id: r.id,
        quantity_sold: r.qty,
        sales_channel: selectedChannel ?? undefined,
      }));
    if (!entries.length) {
    ...
    <View style={{ padding: 16 }}>
          ...
          {menuItemsLoading ? (
            ...
            <ScrollView style={{ maxHeight: 380 }}>
              <View style={{ marginBottom: 8 }}>
                ...
                <Text
                  style={{
                    color: (theme.colors.onSurfaceVariant as string) || '#888',
                    marginBottom: 8,
                  }}
                >
                    ...
                    >
                      ...
                      <PaperInput
                        ...
                        onChangeText={v =>
                          setQtyById(prev => ({
                            ...prev,
                            [m.menu_item_id]: v.replace(/[^0-9]/g, ''),
                          }))
                        }
                        style={{ width: 100 }}
                      />
      ...
      <Portal>
        <Dialog visible={confirmVisible} onDismiss={() => setConfirmVisible(false)}>
          ...
          <Dialog.Content>
            <PaperText>Sales already exist for this date in the following channel(s):</PaperText>
            {Object.entries(conflicts).map(([ch, cnt]) => (
              <PaperText key={ch || 'null'}>
                {ch === 'null' || ch === '' || ch == null ? 'unspecified' : ch} — {String(cnt)}{' '}
                records
              </PaperText>
            ))}
            <PaperText style={{ marginTop: 8 }}>Proceed to overwrite these channel(s)?</PaperText>
          </Dialog.Content>
          <Dialog.Actions>
              ...
              onPress={async () => {
                try {
                  ...
                  const entries = Object.entries(qtyById)
                    .map(([id, val]) => ({
                      id: Number(id),
                      qty: parseInt((val || '0') as string, 10),
                    }))
                    .filter(r => r.id && r.qty && r.qty > 0)
        ...
        <Dialog visible={fileConfirmVisible} onDismiss={() => setFileConfirmVisible(false)}>
          <Dialog.Title>Overwrite existing sales in file?</Dialog.Title>
          <Dialog.Content>
            <PaperText>
              Sales already exist for this date in one or more channel(s) found in your file.
            </PaperText>
            <PaperText style={{ marginTop: 8 }}>
              Proceed to overwrite those channel(s) for the affected date(s)?
            </PaperText>
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={() => setFileConfirmVisible(false)}>Cancel</Button>
            <Button
              onPress={async () => {
