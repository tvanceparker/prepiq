...
import { uploadSalesData, uploadSalesManual, getMenuItems as getMenuItemsApi } from '../../../api/dashboard';
...
export default function BasicOverviewMobile({ data }: Props) {
  ...
  const handleManualSubmit = async () => {
    try {
      ...
      await uploadSalesManual({ sale_date: saleDate, overwrite, entries });
      setSheetOpen(false);
      setQtyById({});
      setSnackbar({ visible: true, message: 'Manual sales saved' });
    } catch (e: any) {
      const msg = String(e?.message || e || 'Manual upload failed');
      const hint = msg.includes('409') ? ' — Data exists. Toggle Overwrite to replace.' : '';
      setSnackbar({ visible: true, message: `${msg}${hint}` });
    }
  };
  return (
