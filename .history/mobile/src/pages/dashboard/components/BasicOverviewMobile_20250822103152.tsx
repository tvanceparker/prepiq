...
import { Button, Snackbar, Dialog, Portal, Text as PaperText, TextInput as PaperInput, useTheme } from 'react-native-paper';
...
export default function BasicOverviewMobile({ data }: Props) {
  ...
  const [conflicts, setConflicts] = useState<Record<string | null, number>>({});
  ...
  const handleManualSubmit = async () => {
    ...
    try {
      ...
      const result = await checkSalesExist(saleDate, ['null']);
      const conf = (result && result.conflicts) || {};
          ...
          <Dialog.Content>
            ...
            {Object.entries(conflicts).map(([ch, cnt]) => (
              <PaperText key={ch || 'null'}>
                {ch || 'unspecified'} — {String(cnt)} records
              </PaperText>
            ))}
