...
import { useMenuItems, MobileMenuItem } from './hooks/useMenuItems.ts';
...
export default function MenuItemQuickEntry() {
  ...
  const filtered = useMemo(()=> menuItems.filter((mi:MobileMenuItem) => {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding:16 }}>
      ...
      {loading ? <ActivityIndicator style={{ marginTop:20 }} /> : (
        <View>
          {filtered.map((mi:MobileMenuItem) => <MenuItemRow key={mi.menu_item_id} item={mi} onDelete={handleDeleteMenuItem} onUpdate={handleUpdateMenuItem} />)}
...
function MenuItemRow({ item, onDelete, onUpdate }:{ item:MobileMenuItem; onDelete:(id:number)=>void; onUpdate:(id:number, data:any)=>void }) {
...
const smallBtn = (bg:string)=> ({ backgroundColor:bg, paddingVertical:6, paddingHorizontal:12, borderRadius:6, marginRight:8, alignItems:'center' as const });
