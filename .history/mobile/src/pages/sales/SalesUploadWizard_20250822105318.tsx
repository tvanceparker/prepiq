...
interface MenuItemLite { menu_item_id: number; name: string }
...
export default function SalesUploadWizard({ navigation }: any) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [menu, settings] = await Promise.all([getMenuItems() as any, getRestaurantSettings() as any]);
  ...
  const visibleItems = useMemo(() => {
    ...
    return base.filter((i) => i.name.toLowerCase().includes(f) || String(i.menu_item_id).includes(filter));
  }, [items, filter]);
  ...
  const submitAll = async (confirmOverwrite: boolean) => {
    ...
    try {
      ...
      await uploadSalesManual({ sale_date, overwrite: !!confirmOverwrite, entries: payloadEntries });
      setSnackbar({ visible: true, message: confirmOverwrite ? 'Sales overwritten' : 'Sales uploaded' });
      navigation.goBack();
    } catch (e: any) {
          ...
          <View>
            {mode === 'byChannel' ? (
              <View>
                ...
                />
                {visibleItems.map((it) => (
                  <View key={it.menu_item_id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              <View style={{ flexDirection: 'row' }}>
                <Button mode="outlined" onPress={goPrev} disabled={currentIndex === 0} style={{ marginRight: 8 }}>
                  Previous
                </Button>
          ...
          <View>
            ...
            {nonZeroEntries.length === 0 ? (
              ...
              nonZeroEntries.map((e) => (
                <View key={`${e.item_id}-${e.channel}`} style={{ flexDirection: 'row', marginBottom: 6 }}>
                  <Text style={{ flex: 2 }}>
                    {e.item_name}
                  </Text>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              <Button mode="contained" onPress={() => submitAll(false)} disabled={nonZeroEntries.length === 0}>
                Submit
              </Button>
            </View>
          </View>
      ...
      <Portal>
        <Dialog visible={confirmVisible} onDismiss={() => setConfirmVisible(false)}>
          ...
          <Dialog.Actions>
            ...
            <Button onPress={() => { setConfirmVisible(false); submitAll(true); }}>Overwrite</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>

      <Snackbar visible={snackbar.visible} onDismiss={() => setSnackbar({ visible: false, message: '' })} duration={2500}>
        {snackbar.message}
      </Snackbar>
    </View>
  );
