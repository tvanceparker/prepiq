...
import { VictoryChart, VictoryLine, VictoryAxis, VictoryTheme, VictoryBar, VictoryPie } from "victory-native";
...
export default function SalesPatternsBasicMobile() {
  ...
  const renderSalesOverTime = () => {
    ...
    return (
      <VictoryChart theme={VictoryTheme.material} height={220} padding={{ left: 50, right: 30, top: 10, bottom: 50 }}>
        <VictoryAxis fixLabelOverlap tickFormat={(t) => `${new Date(t).getMonth()+1}/${new Date(t).getDate()}`} />
    ...
    return (
      <View style={{ flexDirection: "column" }}>
        {heatmapData.slice(0, 10).map((r: any, idx: number) => {
          ...
          const bg = `rgb(${240 - intensity}, ${248 - Math.round(intensity / 2)}, ${255 - intensity})`;
          return (
            <View key={idx} style={{ flexDirection: "row", alignItems: "center", marginBottom: 6 }}>
              <View style={{ width: 12, height: 12, backgroundColor: bg, borderRadius: 2, marginRight: 8 }} />
              <Text style={{ flex: 1 }}>{r.menu_item_name || r.menu_item_id}</Text>
              <Text style={{ width: 80, textAlign: "right" }}>{Math.round(v)}</Text>
            </View>
          );
        })}
        <Text style={{ fontSize: 12, color: "#666", marginTop: 6 }}>Heatmap preview — tap to open detailed view (TBD)</Text>
      </View>
    );
  };
  ...
  const renderWeekdayAvg = () => {
    if (!Array.isArray(weekdayAvg) || weekdayAvg.length === 0)
      return <Text style={{ color: "#555" }}>No data</Text>;
    const data = weekdayAvg.map((r: any) => ({ x: r.weekday, y: Number(r.metric || 0) }));
    return (
      <VictoryChart domainPadding={20} theme={VictoryTheme.material} height={220} padding={{ left: 50, right: 30, top: 10, bottom: 50 }}>
        <VictoryAxis />
        <VictoryAxis dependentAxis />
  ...
  const renderChannelBreakdown = () => {
    if (!Array.isArray(channelBreakdown) || channelBreakdown.length === 0)
      return <Text style={{ color: "#555" }}>No data</Text>;
    const total = channelBreakdown.reduce((s: number, r: any) => s + Number(r.metric || 0), 0) || 1;
    const pieData = channelBreakdown.map((r: any, i: number) => ({ x: r.sales_channel || r.channel || `ch${i}`, y: Number(r.metric || 0) }));
    return (
      <VictoryPie data={pieData} colorScale={COLORS} height={220} labels={({ datum }) => `${datum.x}: ${((datum.y / total) * 100).toFixed(0)}%`} />
    );
  };
...
function Card({ title, children }: { title: string; children: any }) {
  return (
    <View style={{ backgroundColor: "#fff", padding: 12, borderRadius: 10, marginBottom: 14, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6 }}>
...
function Toggle({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  return (
    <TouchableOpacity
      onPress={onPress}
      style={{ backgroundColor: active ? "#2563eb" : "#e5e7eb", paddingVertical: 8, paddingHorizontal: 14, borderRadius: 20, marginRight: 8 }}
    >
