...
export default function MenuMixInsightsBasicMobile() {
  ...
  const {
    ...
    loading,
  } = useMenuMixInsights(startDate, endDate, byRevenue);
        ...
        <>
          ...
          >
            <Text style={{ fontWeight: "600", marginBottom: 6 }}>
              Breakdown ({byRevenue ? "Revenue" : "Qty"})
            </Text>
            {filteredBreakdown.slice(0, 25).map((b: any, i: number) => (
              <View
                key={i}
                style={{
                  flexDirection: "row",
              ...
              >
                <Text>{b.menu_item_name}</Text>
                <Text>
                  {byRevenue ? `$${b.metric?.toFixed?.(2)}` : b.metric}
                </Text>
              </View>
            ))}
          ...
          >
            ...
            {filteredTopBottom.slice(0, 25).map((b: any, i: number) => (
              <View
                key={i}
                style={{
                  flexDirection: "row",
          ...
          >
            ...
            {groupedOverTime.slice(0, 50).map((row: any, i: number) => (
              <View key={i} style={{ marginBottom: 4 }}>
                <Text style={{ fontWeight: "600" }}>{row.date}</Text>
              </View>
            ))}
