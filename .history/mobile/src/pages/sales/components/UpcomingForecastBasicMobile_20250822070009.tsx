...
export default function UpcomingForecastBasicMobile() {
  ...
  const uniqueMenuItems = useMemo(() => {
    ...
    forecastTable.forEach((r: any) => {
      if (r?.menu_item_id)
        map.set(Number(r.menu_item_id), String(r.menu_item_name));
    });
  ...
  const {
    filteredChartData: accuracyChartPoints,
    filteredTableData: accuracyTableRows,
    setSelectedMenuItemIds: setAccuracySelectedIds,
    selectedMenuItemIds: accuracySelectedIds,
    loading: accuracyLoading,
    error: accuracyError,
  } = useForecastAccuracy(isoDate(startDate), isoDate(endDate));
  ...
  const groupedAccuracyOverTime = useMemo(() => {
    ...
    accuracyChartPoints.forEach((p: any) => {
      const dateStr =
        typeof p.date === "string" ? p.date : p.date?.toString?.() || "";
      if (!dateStr) return;
  ...
  const accuracySummary = useMemo(() => {
    ...
    return Object.keys(sums)
      .map((name) => ({
        menu_item_name: name,
        avg_error: sums[name].total / Math.max(1, sums[name].count),
      }))
      ...
      {!loading && !error && (
        <>
                    ...
                    onPress={() => {
                      setSelectedItems((prev) =>
                        prev.includes(Number(id))
                          ? prev.filter((n) => n !== Number(id))
                          : [...prev, Number(id)]
                      );
                      // also apply to accuracy hook (IDs expected)
                      setAccuracySelectedIds((prev) =>
                        prev.includes(Number(id))
            ...
            {forecastTable
              .filter(
                (r: any) =>
                  ...
                  selectedItems.includes(Number(r.menu_item_id))
              )
              .slice(0, 100)
              .map((r: any, i: number) => (
                <View
          ...
          >
            ...
            {accuracyLoading ? (
              <ActivityIndicator />
            ) : groupedAccuracyOverTime.length === 0 ? (
                ...
                <Svg height={240} width="100%" viewBox="0 0 400 240">
                  ...
                  >
                    ...
                    <VictoryAxis
                      ...
                      style={{ tickLabels: { fontSize: 10 } }}
                    />
                    {Object.keys(groupedAccuracyOverTime[0])
                      .filter((k) => k !== "date")
                      .map((key, idx) => (
                        <VictoryLine
                          key={key}
                          data={groupedAccuracyOverTime.map((r: any) => ({
                            x: r.date,
                            y: Number(r[key] || 0),
                          }))}
                          style={{
                            data: {
                              stroke:
                                accuracyColorMap[key] || generateHueColor(idx),
                            },
                          }}
                        />
                      ))}
                      ...
                      >
                        <View
                          style={{
                            ...
                            height: 12,
                            backgroundColor:
                              accuracyColorMap[key] || generateHueColor(i),
                            borderRadius: 3,
                            marginRight: 6,
                          }}
                        />
