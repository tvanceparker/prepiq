...
export default function SalesExplorerBasicMobile() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (form.quantity_sold !== '' && form.quantity_sold !== null && form.quantity_sold !== undefined) {
      ...
      const ts = form.sale_timestamp || editRow.sale_timestamp || editRow.sale_timestamp || editRow.sale_date;
      if (ts) {
        // Backend expects ISO datetime for parsing
        payload.sale_timestamp = typeof ts === 'string' ? ts : new Date(ts).toISOString();
      }
      const mid = form.menu_item_id ?? editRow.menu_item_id ?? editRow.menu_item_id ?? editRow.menu_itemId;
          ...
          <View style={{ width: '100%', maxWidth: 420, backgroundColor: 'white', padding: 12, borderRadius: 10 }}>
            ...
            {form && (
              <>
                ...
                {channelMenuVisible && (
                  <Animated.View style={{ backgroundColor: 'white', borderRadius: 8, marginTop: 8, elevation: 6, transform: [{ scale: menuScale }], opacity: menuOpacity }}>
                    {salesChannels.length === 0 && (
                      <TouchableOpacity onPress={() => setChannelMenuVisible(false)} style={{ padding: 12 }}>
                    ...
                    <TouchableOpacity onPress={() => setChannelMenuVisible(false)} style={{ padding: 12, alignItems: 'center' }}>
                      <Text style={{ color: '#2563eb' }}>Cancel</Text>
                    </TouchableOpacity>
                  </Animated.View>
                )}
