                ...
                <>
                  <Svg height={260} width="100%" viewBox="0 0 400 260">
                    <VictoryPie
                      ...
                      labels={() => null}
                      colorScale={aggregatedBreakdown.map((_, i) => COLORS[i % COLORS.length])}
                    />
                  </Svg>
                  ...
                  <View style={{ marginTop: 8 }}>
                    {aggregatedBreakdown.slice(0, 12).map((b:any, i:number) => (
                      <View key={b.menu_item_id ?? b.menu_item_name} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                        <View style={{ width: 14, height: 14, backgroundColor: COLORS[i % COLORS.length], borderRadius: 3, marginRight: 8 }} />
                        <Text style={{ flex: 1 }}>{b.menu_item_name}</Text>
                        <Text style={{ fontWeight: '600' }}>{byRevenue ? `$${Number(b.metric).toLocaleString()}` : Number(b.metric).toLocaleString()}</Text>
                      </View>
                    ))}
                  </View>
                </>
