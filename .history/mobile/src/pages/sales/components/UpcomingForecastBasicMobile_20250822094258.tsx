...
export default function UpcomingForecastBasicMobile() {
  ...
  {error && <Text style={{ color: theme.colors.error }}>Failed to load forecast</Text>}
      {!loading && !error && (
        <>
          ...
          >
            ...
            {mode === 'per_day' ? (
              ...
              <>
                ...
                <Text style={{ fontWeight: '600' }}>
                  Total Revenue: ${(forecastTotals?.forecasted_revenue || 0).toFixed(2)}
                </Text>
                <ScrollView
                  style={{ maxHeight: 200 }}
                  nestedScrollEnabled
                ...
                >
                  {totalItemsSummary?.map(i => (
                    <Text key={i.name} style={{ fontSize: 12, color: theme.colors.onSurfaceVariant }}>
                      • {i.name}: {i.quantity}
                    </Text>
                  ))}
                </ScrollView>
            ...
            {forecastTable
              .filter(
                (r: any) =>
                  ...
                  selectedItems.includes(String(r.menu_item_name))
              )
              .slice(0, 100)
              .map((r: any, i: number) => (
                <View
...
function Toggle({
  ...
  return (
    ...
    >
      <Text style={{ color: active ? theme.colors.onPrimary : theme.colors.onSurface, fontSize: 12 }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
