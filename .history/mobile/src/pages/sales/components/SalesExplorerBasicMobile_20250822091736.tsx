    ...
    >
      ...
      <View style={{ marginBottom: 12, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <Menu
          ...
          anchor={<Button mode="outlined" onPress={() => setMenuVisible(true)}>Menu Item</Button>}
        >
          <Menu.Item title="All Items" onPress={() => { setMenuItemId(null); setMenuVisible(false); }} />
          {menuItems.map((mi: any) => (
            <Menu.Item
              key={mi.id || mi.menu_item_id}
              title={mi.name || mi.menu_item_name || String(mi.id)}
              onPress={() => { setMenuItemId(mi.id || mi.menu_item_id); setMenuVisible(false); }}
            />
          ))}
        </Menu>
