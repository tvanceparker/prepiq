...
export default function UpcomingForecastBasicMobile(){
  ...
  const uniqueMenuItems = useMemo(() => {
    ...
    forecastTable.forEach((r:any) => { if (r?.menu_item_name) names.add(String(r.menu_item_name)); });
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
                ...
                {uniqueMenuItems.map(name => (
                  <Chip
                    ...
                    onPress={()=> setSelectedItems(prev => prev.includes(name) ? prev.filter(n => n!==name) : [...prev, name])}
                    style={{ marginRight: 8 }}
                  >{name}</Chip>
                ))}
              </ScrollView>
