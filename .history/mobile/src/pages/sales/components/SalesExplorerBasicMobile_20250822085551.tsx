...
export default function SalesExplorerBasicMobile() {
  ...
  const submit = async () => {
    if (!form.menu_item_id || !form.sale_timestamp || !form.quantity_sold || !form.sales_channel) {
    ...
    try {
      await createSaleRecord({
        ...
        sale_timestamp: new Date(form.sale_timestamp).toISOString(),
      ...
      {openForm && (
        ...
        >
          ...
          <TextInput
            ...
            value={form.menu_item_id ? String(form.menu_item_id) : ''}
            onChangeText={t => setForm(f => ({ ...f, menu_item_id: Number(t) || null }))}
            style={ti}
          />
          <TextInput
            placeholder="Timestamp"
            value={form.sale_timestamp}
            onChangeText={t => setForm(f => ({ ...f, sale_timestamp: t }))}
            style={ti}
          />
          <TextInput
            placeholder="Quantity"
            value={String(form.quantity_sold)}
            onChangeText={t => setForm(f => ({ ...f, quantity_sold: Number(t) || 0 }))}
            style={ti}
          ...
          <TextInput
            ...
            onChangeText={t => setForm(f => ({ ...f, sales_channel: t }))}
            style={ti}
          />
          <TextInput
            placeholder="Revenue"
            value={String(form.revenue)}
            onChangeText={t => setForm(f => ({ ...f, revenue: Number(t) || 0 }))}
            style={ti}
            keyboardType="numeric"
          />
          <TouchableOpacity
      ...
      {!loading && !error && (
        <View
          style={{
            ...
            padding: 12,
            borderRadius: 12,
            // keep the sales card from growing indefinitely (prevents overflow from large children like charts)
            maxHeight: 420,
            overflow: 'hidden',
          }}
        >
          ...
          {/* Inner scroll for the list so the card keeps stable size */}
          <ScrollView style={{ maxHeight: 360 }} nestedScrollEnabled>
            {data.length > 0 ? (
              data.slice(0, 200).map((row: any, i: number) => (
                <View
                  key={row.sale_id ?? row.sale_timestamp ?? i}
                  style={{
                    flexDirection: 'row',
                ...
                >
                  <Text style={{ flex: 1, flexShrink: 1 }} numberOfLines={1} ellipsizeMode="tail">
                    {row.menu_item_name || row.menu_item_id}
                  </Text>
                  <Text style={{ width: 70, textAlign: 'right' }}>{row.quantity_sold}</Text>
                </View>
              ))
            ) : (
              <Text style={{ fontSize: 12, color: '#555' }}>No records.</Text>
            )}
          </ScrollView>
        </View>
      )}
