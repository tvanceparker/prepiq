...
export default function ForecastAccuracyBasicMobile() {
  ...
  // Convert error% -> accuracy% (accuracy = 100 - error)
  const grouped = useMemo(() => {
    ...
    filteredChartData.forEach((p: any) => {
      ...
      if (!d) return;
      if (!g[d]) g[d] = { date: d };
      const rawError = Number(p.error_percentage ?? 0);
      const acc = 100 - rawError;
    ...
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      ...
      {!loading && !error && (
        <>
          ...
          <View style={{ backgroundColor: theme.colors.surface, padding: 12, borderRadius: 12, marginBottom: 16 }}>
            <Text style={{ fontWeight: '600', marginBottom: 8 }}>Forecast Accuracy Percentage</Text>
            {grouped.length === 0 ? (
                  ...
                  >
                    ...
                    {Object.keys(grouped[0])
                      ...
                      .map((key, idx) => (
                        <VictoryLine
                          key={key}
                          data={grouped.map((r: any) => ({ x: r.date, y: Number(r[key] || 0) }))}
      ...
      <View style={{ backgroundColor: theme.colors.surface, padding: 12, borderRadius: 12, marginBottom: 16 }}>
        ...
        <Text style={{ fontSize: 12, color: theme.colors.onSurfaceVariant }}>No rows.</Text>
              )}
              {filteredTableData.map((row: any, i: number) => {
                const rawError = Number(
                  row.error_percentage ?? row.error ?? row.accuracy_percent ?? row.accuracy ?? 0
                );
                const acc = Math.max(0, Math.min(100, 100 - rawError));
                return (
                  <View
                    key={i}
                    style={{
                  ...
                  >
                    ...
                    <Text
                      style={{ width: 60, textAlign: 'right', fontWeight: '600' }}
                    >{`${acc.toFixed(1)}%`}</Text>
                  </View>
                );
              })}
