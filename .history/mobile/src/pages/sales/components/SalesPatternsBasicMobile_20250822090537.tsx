...
export default function SalesPatternsBasicMobile() {
  ...
  // pick top 3 items by total
    ...
    const xMin = isValidDate(startDate) ? new Date(startDate) : new Date(salesOverTime[0].sale_date || salesOverTime[0].date);
    const xMax = isValidDate(endDate) ? new Date(endDate) : new Date(salesOverTime[salesOverTime.length - 1].sale_date || salesOverTime[salesOverTime.length - 1].date);
    ...
    return (
      <View style={{ height: 260, overflow: 'hidden' }}>
        ...
        >
          ...
          />
          <VictoryAxis dependentAxis tickFormat={v => (Math.abs(v) >= 1000 ? `${Math.round(v)}` : `${Math.round(v)}`)} />
          {top.map((k, i) => {
  ...
  const renderHeatmapPreview = () => {
    ...
    if (list.length === 0) return <Text style={{ color: '#555' }}>No data</Text>;
    const data = list.map((r: any) => ({ x: r.menu_item_name || String(r.menu_item_id), y: safeNum(r.metric) }));
  ...
  const renderWeekdayAvg = () => {
    ...
    const data = weekdayAvg
      .map((r: any) => ({ x: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][r.weekday] ?? String(r.weekday), y: safeNum(r.metric) }))
    ...
    return (
      <View style={{ height: 220, overflow: 'hidden' }}>
        <VictoryChart domainPadding={20} theme={VictoryTheme.material} height={220} padding={{ left: 40, right: 30, top: 10, bottom: 50 }} domain={{ y: [0, yMax] }}>
          <VictoryAxis />
  ...
  const renderChannelBreakdown = () => {
    ...
    return (
      <View style={{ height: 220, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' }}>
        <VictoryPie
          data={pieData}
          colorScale={COLORS}
          height={200}
