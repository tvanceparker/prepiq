...
export const checkSalesExist = async (
  ...
  const base = process.env.API_BASE_URL || 'http://10.0.2.2:8000/api/v1';
  const params = new URLSearchParams({ sale_date });
  if (channels && channels.length) {
    channels.forEach(ch => params.append('channels', ch ?? 'null'));
  }
  const res = await fetch(`${base}/dashboard/sales-exist?${params.toString()}`);
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`${res.status}: ${err}`);
  }
