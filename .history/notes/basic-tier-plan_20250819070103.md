...
- [ ] Consider building a small PrepIQ POS later if uptake requires tight integration
 - [ ] Add payments integration: Stripe for card payments, refunds, and receipts
   - Design a `payments` service and API surface for creating charges, handling webhooks, and storing payment metadata per `restaurant_id`
   - Use Stripe Checkout / Payment Intents for PCI compliance and strong SCA handling
   - Consider Stripe Connect for multi-restaurant onboarding (separate accounts or transfers)
   - Plan webhooks for successful payments, refunds, disputes, and payout notifications; persist events in `payments`/`payment_events` tables
   - Add a small POS adapter that can 1) open/close orders, 2) submit sales (to `/integrations/pos/ingest`), and 3) request a payment via Stripe; receipts stored and linked to sales rows
 - [ ] Decide deployment model: offer POS as cloud-hosted service + lightweight kiosk client (Electron/React) or as Dockerized self-host for enterprise
...
6. Add DB tables + repos for `weather_data` and `traffic_data` and an ingestion/backfill job
7. Wire weather/traffic features into `train_model` feature engineering and add tests
8. Add an integration test that calls `GET /api/v1/dashboard/sales-upload-template` and verifies header
9. Add OpenAPI → TS types generation for dashboard endpoints
10. Add seed scripts and a README onboarding doc for a demo restaurant
11. Add CI pipeline for backend tests + frontend build
12. Sketch Expo scaffold and shared JS module plan for mobile
13. Design and implement POS + Stripe integration (adapter, payments service, webhooks, receipts, basic kiosk client)

### Frontend typing and data-model refactor
- [ ] Migrate `frontend` to TypeScript (incremental plan)
  - Start by enabling `allowJs` + `checkJs` and add `tsconfig.json` with `jsx: react-jsx` then rename high-value components (`SummaryCard`, `BasicOverview`, `TopItems`) to `.tsx`
  - Add types for API responses by generating types from OpenAPI or by hand for key endpoints (dashboard, forecasts, sales-upload)
  - Add strict linting rules and enable typechecks in CI for the frontend build
- [ ] Audit backend data models and normalize where needed
  - Identify model drift introduced while learning: common issues are nullable vs optional fields, snake_case vs camelCase, and denormalized aggregates
  - Add simple migration plan: create new columns/tables, backfill from existing columns, then switch service code to use new models and drop old columns

### POS & Payments roadmap (brief)
- MVP (Phase 1)
  - Payments: integrate Stripe Payment Intents and webhooks; record `payment_id`, `amount`, `currency`, `status`, and `restaurant_id` on successful payments
  - POS ingest: accept batched sales and map POS order_id -> internal sales rows; support refunds via API
  - Kiosk client: simple Electron or React app to capture orders and open a Checkout session
- Phase 2
  - Stripe Connect onboarding, payouts, and revenue reporting per restaurant
  - Reconciliation: daily payout reconciliation between sales and Stripe payouts
  - Add offline-first behavior for kiosk (store locally, sync on network)
