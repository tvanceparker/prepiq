...
export const updateMenuItem = (menuItemId: string | number, data: any) => put(`/dashboard/update/${menuItemId}`, data);
export const deleteMenuItem = (menuItemId: string | number) => del(`/dashboard/delete/${menuItemId}`);

export const uploadMenuCSV = async (file: any) => {
  const formData = new FormData();
  formData.append('file', file);
  const res = await fetch(`${process.env.API_BASE_URL || 'http://10.0.2.2:8000/api/v1'}/dashboard/upload-csv`, {
    method: 'POST',
    body: formData,
  });
  if (!res.ok) {
...
export const downloadSalesTemplate = async (defaultDate?: string) => {
  let url = `${process.env.API_BASE_URL || 'http://10.0.2.2:8000/api/v1'}/dashboard/sales-upload-template`;
...
export const uploadSalesData = async (file: any, overwrite = false) => {
  ...
  const res = await fetch(`${process.env.API_BASE_URL || 'http://10.0.2.2:8000/api/v1'}/dashboard/upload-sales-data`, { method: 'POST', body: formData });
  if (!res.ok) {
    const err = await res.text();
