...
export const createMenuItem = (data: Partial<MenuItemDTO>) => post('/dashboard/create_menu_item', data);
export const updateMenuItem = (menuItemId: number | string, data: Partial<MenuItemDTO>) =>
  put(`/dashboard/update/${menuItemId}`, data);
export const deleteMenuItem = (menuItemId: number | string) => del(`/dashboard/delete/${menuItemId}`);

export const uploadMenuCSV = async (file: File) => {
  const formData = new FormData();
  formData.append('file', file);

  const res = await authFetch(`${BASE_URL}/dashboard/upload-csv`, { method: 'POST', body: formData });
  if (!res.ok) {
...
export const uploadSalesData = async (file: File, overwrite = false) => {
  ...
  const res = await authFetch(`${BASE_URL}/dashboard/upload-sales-data`, { method: 'POST', body: formData });
  if (!res.ok) {
    const err = await res.text();
...
export const uploadSalesManual = (payload: EodSalesEntriesInDTO) => post('/dashboard/upload-sales-manual', payload);
