...
export default function PrepSchedule() {
  ...
  const { create: createPrep, loading: loadingCreate } = useCreatePrepSchedule();
  const { update: updatePrep, loading: loadingUpdate } = useUpdatePrepSchedule();
  ...
  const handleCreatePrepSchedule = async () => {
    ...
    if (!createQuantity || isNaN(qty) || qty <= 0) {
      setSnackbar({
        open: true,
  ...
  const handleUpdatePrepSchedule = async () => {
    if (updateStatus === 'completed') {
      ...
      if (!updateTime || isNaN(time) || time <= 0) {
        setSnackbar({
          open: true,
      ...
      const batchCount = Number(updateBatchCount);
      if (!updateBatchCount || isNaN(batchCount) || batchCount <= 0) {
        setSnackbar({
          open: true,
    ...
    try {
      await updatePrep({
        ...
        status: updateStatus,
        prep_time_minutes_actual: updateStatus === 'completed' ? Number(updateTime) : null,
        prep_batch_count: updateStatus === 'completed' ? Number(updateBatchCount) : null,
      });
      setSnackbar({
        open: true,
  ...
  const loading = loadingBatchRecipes || loadingPrepSchedules || loadingCreate || loadingUpdate;
  ...
  if (errorBatchRecipes || errorPrepSchedules) {
    return (
      <Box sx={{ mt: 5 }}>
        <Alert severity="error">Failed to load data. Please refresh or try again later.</Alert>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: 4 }}>
              ...
              >
                <CardContent>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Estimated Prep Time: {batch.estimated_prep_time_minutes ?? '-'} mins
                  </Typography>
                </CardContent>
              </Card>
            </Grid>
      ...
      <Dialog open={createDialogOpen} onClose={handleCloseCreateDialog} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <Typography variant="body2" gutterBottom>
            Yield: {selectedBatchRecipe?.yield_quantity} {selectedBatchRecipe?.yield_unit}
          </Typography>
          <TextField
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleCreatePrepSchedule} disabled={loadingCreate}>
            Create
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={updateDialogOpen} onClose={handleCloseUpdateDialog} maxWidth="xs" fullWidth>
        <DialogTitle>Update Prep Schedule</DialogTitle>
        <DialogContent>
          <Typography variant="subtitle1" gutterBottom>
            {selectedPrepSchedule?.batch_recipe_name} - {selectedPrepSchedule?.prep_date}
          </Typography>
          <Typography variant="body2" gutterBottom>
            Current Status: <Chip label={selectedPrepSchedule?.status} size="small" />
          </Typography>

          {/* Status toggle */}
          <Box sx={{ mt: 1, mb: 2 }}>
            <Button
              variant={updateStatus === 'in_progress' ? 'contained' : 'outlined'}
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleUpdatePrepSchedule} disabled={loadingUpdate}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
