...
function IngredientTrends() {
  ...
  const filteredIngredients = useMemo(() => {
    return fakeIngredientCosts.filter(ingredient => {
      ...
      const matchesSupplier = filters.supplier ? ingredient.supplier === filters.supplier : true;
      return matchesCategory && matchesSupplier;
    });
  }, [filters]);

  const selectedIngredient = filteredIngredients.find(i => i.id === selectedIngredientId) || null;
        ...
        <table className="min-w-full table-auto border-collapse border border-gray-300">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="border border-gray-300 px-3 py-2 text-left">Supplier</th>
              <th className="border border-gray-300 px-3 py-2 text-right">Latest Cost ($)</th>
              <th className="border border-gray-300 px-3 py-2 text-right">% Change (12 wks)</th>
              <th className="border border-gray-300 px-3 py-2 text-center">View Trend</th>
            </tr>
          </thead>
          <tbody>
            {filteredIngredients.length === 0 ? (
              ...
              filteredIngredients.map(ingredient => {
                ...
                return (
                  <tr key={ingredient.id} className="hover:bg-gray-50 cursor-pointer">
                    <td className="border border-gray-300 px-3 py-1">{ingredient.name}</td>
                    <td className="border border-gray-300 px-3 py-1">{ingredient.category}</td>
                    <td className="border border-gray-300 px-3 py-1">{ingredient.supplier}</td>
                    <td className="border border-gray-300 px-3 py-1 text-right">
                      ${ingredient.weeklyCosts[ingredient.weeklyCosts.length - 1].toFixed(2)}
                    </td>
                    <td
                      className={`border border-gray-300 px-3 py-1 text-right ${
                        change > 10 ? 'text-red-600' : change < -10 ? 'text-green-600' : ''
                      }`}
                    >
                      {change.toFixed(1)}%
                    </td>
