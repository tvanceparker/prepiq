...
const calculateDishProfit = (dish) => {
  ...
  const profitMargin =
    dish.salesRevenue > 0 ? (profit / dish.salesRevenue) * 100 : 0;
  return { profit, profitMargin };
};

const uniqueCategories = [
  ...new Set(fakeDishProfitData.map((d) => d.category)),
];
...
function DishProfitability() {
    ...
    filteredDishes.length > 0
      ? (
          filteredDishes.reduce(
            (sum, d) => sum + calculateDishProfit(d).profitMargin,
            0
  ...
  const handleFilterChange = (field) => (e) => {
    const value =
      field === "minSalesCount" ? Number(e.target.value) : e.target.value;
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white rounded shadow-md">
      ...
      <div className="mb-6 text-lg font-semibold flex flex-wrap gap-6">
        ...
        <div>
          ...
          <span className="text-green-600">{avgMargin}%</span>
        </div>
