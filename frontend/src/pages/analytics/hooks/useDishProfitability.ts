import { useMemo, useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { DishProfitabilityItem, DishProfitabilityResponse } from '../../../interfaces/analytics';
import { getDishProfitability } from '../../../api/profitAnalytics';

const iso = (d: Date) => d.toISOString().slice(0, 10);
const today = new Date();
const defaultStart = new Date();
defaultStart.setDate(today.getDate() - 30);

type SortKey = 'margin' | 'foodCost' | 'revenue';

export function useDishProfitability() {
  const [startDate, setStartDate] = useState<string | undefined>(iso(defaultStart));
  const [endDate, setEndDate] = useState<string | undefined>(iso(today));
  const [search, setSearch] = useState('');
  const [category, setCategory] = useState('');
  const [sortKey, setSortKey] = useState<SortKey>('margin');

  const query = useQuery<DishProfitabilityResponse>({
    queryKey: ['dish-profitability', startDate, endDate],
    queryFn: () =>
      getDishProfitability({
        start_date: startDate,
        end_date: endDate,
      }),
    enabled: Boolean(startDate && endDate),
  });

  const data = query.data as DishProfitabilityResponse | undefined;

  const items = useMemo(() => {
    const base = data?.items || [];
    const filtered = base.filter(item => {
      const matchCategory = category ? item.category === category : true;
      const matchSearch = search ? item.name.toLowerCase().includes(search.toLowerCase()) : true;
      return matchCategory && matchSearch;
    });
    const sorter: Record<SortKey, (a: DishProfitabilityItem, b: DishProfitabilityItem) => number> =
      {
        margin: (a, b) => b.gross_margin - a.gross_margin,
        foodCost: (a, b) => a.food_cost_pct - b.food_cost_pct,
        revenue: (a, b) => (b.revenue || 0) - (a.revenue || 0),
      };
    return [...filtered].sort(sorter[sortKey]);
  }, [category, data, search, sortKey]);

  const categories = useMemo(() => {
    const seen = new Set<string>();
    (data?.items || []).forEach(item => {
      if (item.category) seen.add(item.category);
    });
    return Array.from(seen);
  }, [data]);

  const summary = useMemo(() => {
    const totalMargin = items.reduce((sum, item) => sum + item.gross_margin, 0);
    const totalFoodCostPct = items.reduce((sum, item) => sum + item.food_cost_pct, 0);
    return {
      avgMargin: items.length ? totalMargin / items.length : 0,
      avgFoodCostPct: items.length ? totalFoodCostPct / items.length : 0,
    };
  }, [items]);

  const topBottom = useMemo(() => {
    const sortedByMargin = [...items].sort((a, b) => b.gross_margin - a.gross_margin);
    return {
      top: sortedByMargin.slice(0, 3),
      bottom: sortedByMargin.slice(-3).reverse(),
    };
  }, [items]);

  const setQuickRange = (days: number) => {
    const end = new Date();
    const start = new Date();
    start.setDate(end.getDate() - days);
    setStartDate(iso(start));
    setEndDate(iso(end));
  };

  return {
    startDate,
    endDate,
    setStartDate,
    setEndDate,
    search,
    setSearch,
    category,
    setCategory,
    sortKey,
    setSortKey,
    query,
    data,
    items,
    categories,
    summary,
    topBottom,
    setQuickRange,
  };
}
