...
export default function BasicRestaurantSettings() {
  const { settings, loading, error, saveSettings, saving } =
    ...
    >
      <Paper elevation={3} sx={{ p: 3 }}>
        ...
        <Stack spacing={1} color="text.primary">
          ...
          <Typography>
            <strong>Run EOD When Closed:</strong>{" "}
            {settings.eod_run_when_closed ? "Yes" : "No"}
          </Typography>
          <Typography>
            <strong>EOD Buffer Time:</strong>{" "}
            {settings.eod_run_after_close_mins} minutes
          </Typography>
          <Typography>
            <strong>Sales Channels:</strong>{" "}
            {settings.sales_channels?.length
              ? settings.sales_channels.join(", ")
              : "None"}
          </Typography>
        </Stack>
      </Paper>
