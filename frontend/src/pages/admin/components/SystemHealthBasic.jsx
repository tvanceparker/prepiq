...
export function SystemHealthBasic({ initialDate }) {
  ...
  const { overall_status, ...checks } = data;

  const tableData = Object.entries(checks).map(([key, { exists }]) => ({
    id: key,
  ...
  return (
    ...
    >
      ...
      >
        After running <strong>Run Sales Data Check</strong>, go to alerts to see
        what is wrong with the sales data you uploaded. You can fix issues from
        there. Make sure you have your past sales history ready.
      </HintBox>
    </Paper>
  );
}
