...
export default function RolesPermissionsBasic() {
  const { roles, permissions, loading, error, syncData, deleteRole } = useRolePermissions();
  ...
  const [confirmAction, setConfirmAction] = useState<((payload?: any) => void) | null>(null);
  ...
  const saveChanges = async () => {
    try {
      const payload = {
        roles: localRoles.map(({ role_id, name, description, permission_names }) => ({
          ...(role_id && { role_id }),
          name,
          description,
          permission_names,
  ...
  return (
    ...
    >
      ...
      <Stack direction="row" spacing={2} mb={4}>
        ...
        <Button muiVariant="outlined" variant="cancel" showIcon={false} onClick={cancelChanges}>
          Cancel
        </Button>
      </Stack>
