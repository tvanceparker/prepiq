...
export default function BasicUpcomingForecast() {
  ...
  const totalItemsSummary = useMemo(() => {
    ...
    forecastTable.forEach(({ menu_item_name, forecasted_quantity }) => {
      ...
      itemSums[menu_item_name] =
        (itemSums[menu_item_name] || 0) + forecasted_quantity;
    });
      ...
      <Paper elevation={3} sx={{ py: 2, px: 4, mb: 5, borderRadius: 2 }}>
        ...
        >
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Button
              variant={mode === "per_day" ? "primary" : "muted"}
              onClick={() => setMode("per_day")}
            >
              Per Day
            </Button>
            <Button
              variant={mode === "total" ? "primary" : "muted"}
      ...
      {!loading && !error && (
        ...
        >
          ...
          >
            ...
            {mode === "per_day" ? (
              !forecastTotals || forecastTotals.length === 0 ? (
      ...
      {!loading && !error && (
        <>
          ...
          <MaterialReactTable
            ...
            renderGroupedCell={({ cell, table }) => {
              const dateValue = cell.getValue();
              return (
                <Typography
                  sx={{ cursor: "pointer", userSelect: "none" }}
                  onClick={() => table.toggleRowExpanded(cell.row.id)}
                >
