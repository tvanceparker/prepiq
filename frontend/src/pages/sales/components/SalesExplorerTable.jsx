  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const originalRow = tableData.find(
        (item) => item.sale_id === editingRowId
      );
      ...
      setTableData((prev) =>
        prev.map((item) =>
          item.sale_id === editingRowId ? { ...item, ...payload } : item
        )
      );
  ...
  return (
    <>
      <MaterialReactTable
        ...
        localization={{ noRowsLabel: "No sales data available." }}
      ...
      >
        ...
        >
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Sales Channel" />
            )}
            sx={{ width: 180 }}
            disableClearable
          />
