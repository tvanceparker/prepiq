...
import type { SalesPatternVisualizerData, SalesPatternMatrixRow } from '../../../interfaces/sales';

// Create matrix for heatmap table
const createHeatmapMatrix = (
  data: SalesPatternVisualizerData[]
): { matrix: SalesPatternMatrixRow[]; dates: string[] } => {
  const dates: string[] = [...new Set(data.map(d => d.date))].sort();
  const items: string[] = [...new Set(data.map(d => d.menu_item_name))];

  const matrix: SalesPatternMatrixRow[] = items.map(item => {
    const row: SalesPatternMatrixRow = { menu_item_name: item };
    dates.forEach((date: string) => {
      const match = data.find(d => d.menu_item_name === item && d.date === date);
...
export default function SalesPatternsVisualizer({ data }) {
  ...
  const { matrix, dates } = useMemo(() => createHeatmapMatrix(byItem), [byItem]);

  const barCategoryData = useMemo(() => {
    const aggregated = {};
    for (const entry of byCategory) {
      aggregated[entry.category] = (aggregated[entry.category] || 0) + entry.value;
    }
  ...
  const barItemData = useMemo(() => {
    ...
    for (const entry of byItem) {
      aggregated[entry.menu_item_name] = (aggregated[entry.menu_item_name] || 0) + entry.value;
    }
      ...
      <Box sx={{ flex: 1, minHeight: 0 }}>
        ...
        {view === 'bar-category' && (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={barCategoryData} margin={{ top: 20, right: 20, left: 10, bottom: 5 }}>
        ...
        {view === 'bar-item' && (
          <Box sx={{ width: '100%', height: '100%', overflowX: 'auto' }}>
            <Box sx={{ minWidth: barItemData.length * 60, height: '100%' }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barItemData} margin={{ top: 20, right: 20, left: 10, bottom: -5 }}>
                  <CartesianGrid stroke={borderColor} strokeDasharray="3 3" />
                  <XAxis
        ...
        {view === 'heatmap' && (
          ...
          >
            <Table stickyHeader size="small" sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                {matrix.map(row => {
                  const maxVal = Math.max(...dates.map(d => Number(row[d]) || 0));
                  return (
                    <TableRow key={row.menu_item_name} hover>
                      ...
                      {dates.map(date => {
                        ...
                        const alpha = maxVal === 0 ? 0 : 0.1 + (val / maxVal) * 0.9;
